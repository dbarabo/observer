...
object RemartMail : FileFinder, FileProcessor {

    override val fileFinderData: List<FileFinderData> =
            listOf(FileFinderData("D:/Arhiv_in/GUPRIM", "\\d\\d\\d\\d\\.\\d\\d\\.\\d\\d\\.\\d\\d\\d\\d\\d\\d_.*"))
    ...
    override fun name(): String = "Переслать Ремарт"
    ...
    override fun processFile(file :File) {
        ...
        var destinationFile = File("${directoryTo.absolutePath}/${file.name.substring(18)}")
        if(destinationFile.exists()) {
            destinationFile =  File("${directoryTo.absolutePath}/${file.name}")
        }
