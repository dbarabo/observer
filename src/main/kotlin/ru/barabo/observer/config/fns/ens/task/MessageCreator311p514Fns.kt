...
import ru.barabo.observer.config.skad.crypto.p311.TypeFace
import ru.barabo.observer.config.skad.crypto.p311.validateXml
import ru.barabo.observer.config.task.p311.v512.AccountClose
...
import ru.barabo.observer.config.task.p311.v512.NpIP
import ru.barabo.observer.config.task.p311.v512.NpRo
import ru.barabo.observer.config.task.p311.v512.SvAccount
import ru.barabo.observer.config.task.p311.v512.SvBank
import ru.barabo.observer.config.task.p311.v512.SvNp
...
private val logger = LoggerFactory.getLogger(MessageCreator311p514Fns::class.java)

object MessageCreator311p514Fns {
    ...
    private fun saveXmlFile(idMessage: Number, mainFileData: MainFile): File {
        ...
        val xsd = when (fileName.substring(0..2) ) {
            "FRS" -> "/xsd/sfr/FRS0_101.xsd"
            "SBC" -> "/xsd/fns311p/SBC0_514.xsd"
            "SFC" -> "/xsd/fns311p/SFC0_514.xsd"
    ...
    private fun fullFile(fileName: String): File {
        ...
        val folder = if (fileName.isSfr() ) sfrFolderSent() else fnsFolderSent()
